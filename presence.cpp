...
switch(restCode){
 ...
 case 0:
   ...
   break;
 case -1:
   callback(context, channel, (char*)"Can not connect to server!", NULL);
   break;
 case -2:
   callback(context, channel, (char*)"Can not init curl!", NULL);
   break;
 case -3:
   callback(context, channel, (char*)"Malloc() failed!", NULL);
   break;
  }
}
...
void *_ortc_presence(void *ptr){
  ...
  if(url){
    ...
    if(ret < 1){
      switch(ret){
      case -3:
	p->callbackGet(p->context, p->channel, (char*)"malloc() failed!", NULL);
	break;
      case -2:
	p->callbackGet(p->context, p->channel, (char*)"Can not init curl!", NULL);
	break;
      case -1:
	p->callbackGet(p->context, p->channel, (char*)"Can not connect to server!", NULL);
	break;
      case 0:
	...
	break;
      ...
      if(presence == NULL)
	p->callbackGet(p->context, p->channel, (char*)"Error parsing a server response", NULL);
      else {
...
ortc_presenceData* _ortc_parsePresence(char *p){
  ...
  if(p[i]==','){//there is a metadata
	...
	if(isCM==0){
	  ...
	  cm[i - cmStartIdx - 1] = '\0';
	  replaced = NULL;
	  replaced = (char*)_ortc_replace(cm, (char*)"\\\\", (char*)"\\");
	  currentMetadata = NULL;
	  currentMetadata = (char*)_ortc_replace(replaced, (char*)"\\\"", (char*)"\"");
	  free(cm);
	  free(replaced);
	}
      } else if(p[i]==':' && isCM==0) {
